			...
			.each(
					...
					function(index, semester) {
						var html = '<tr data-id="'
								...
								+ '<td><a class="btn btn-info glyphicon glyphicon-download-alt" href="/user/1234567890/major/courseAccounts/excel?type=previous&semester='
								+ semester.id + '"></a></td>' + '</tr>';
	...
	$('#semester-content td .glyphicon-eye-open').on(
			...
			function() {
				...
				var target = new CourseAccountTargetObject(
						...
						'/api/user/1234567890/major/courseAccounts');
	...
	this.pageInfoParser = function(jsonPageInfo) {
		...
		$.each(jsonPageInfo.courseAccounts.pageList, function(index, account) {
			...
			var html = '<tr data-id="' + account.id + '">' + '<td>'
					...
					// glyphicon-eye-open"aria-hidden="true" data-toggle="modal"
					// data-target="#courseAccountModal"></span></td>'
					+ '</tr>';
	...
	$('#courseAccount-content td .glyphicon-eye-open')
			.on(
					...
					function() {
						...
						ajaxAction({
							url : '/api/user/1234567890/major/courseAccount/'
									+ accountId,
							method : 'GET',
						}, {
